...
typedef std::function<void(const uint8_t* data,const std::size_t data_length, int32_t width, int32_t height)> NEW_FRAME_CALLBACK;
...
public:
    ...
    VideoDecoder(NEW_FRAME_CALLBACK onNewFrame);
...
private:
    ...
    const NEW_FRAME_CALLBACK onNewFrame;
private:
